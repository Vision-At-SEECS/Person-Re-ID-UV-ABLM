...
class GaussianKernel(torch.nn.Module):
    ...
    def forward(self, x_unf, w, b):
        l = x_unf.transpose(1, 2)[:, :, :, None] - w.view(1, 1, -1, w.size(0))  # x_unf[:, :, :, None].transpose(1, 2) - w.view(w.size(0), 1, 1, -1).transpose(0,1)   
        l = torch.sum(l**2, 2)   # torch.sum(l**2, 2)
...
class KernelConv2d(torch.nn.Conv2d):
    def __init__(self, in_channels, out_channels, kernel_size=3, kernel_fn=PolynomialKernel,
  #  def __init__(self, in_channels, out_channels, kernel_size=3, kernel_fn=GaussianKernel,
                 stride=1, dilation=1, padding=0, groups=1, bias=None):
                 #stride=1, dilation=1, padding=1, groups=1, bias=None):
        '''
